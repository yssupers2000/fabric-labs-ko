...
1. 원하는 이름으로 새 워크스페이스를 만들고 Fabric Capacity(*Trial*, *Premium*, 또는 *Fabric*)를 포함하는 라이선스 모드를 선택합니다.
...
1. 첫 번째 cell (현재 *코드* cell입니다)을 선택한 다음, 오른쪽 상단의 동적 도구 모음에서 **M&#8595;** 버튼을 사용하여 cell을 *markdown* cell로 변환합니다.

   cell이 markdown cell로 변경되면 해당 텍스트가 렌더링됩니다.

1. **&#128393;** (편집) 버튼을 사용하여 cell을 편집 모드로 전환한 다음, 내용을 삭제하고 다음 텍스트를 입력합니다.
...
1. cell 왼쪽에 있는 **&#9655; Cell 실행** 버튼을 사용하여 실행합니다. 또는 키보드에서 `SHIFT` + `ENTER`를 눌러 cell을 실행할 수 있습니다.

   > **참고**: 이 세션에서 Spark 코드를 처음 실행하는 것이므로 Spark pool이 시작되어야 합니다. 이는 세션의 첫 번째 실행에 완료하는 데 1분 정도 걸릴 수 있음을 의미합니다. 이후 실행은 더 빨라집니다.

1. cell 출력 아래에 있는 **+ 코드** 아이콘을 사용하여 Notebook에 새 코드 cell을 추가하고 다음 코드를 입력합니다.

    ```python
   display(df)
    ```

1. cell 명령이 완료되면 cell 아래의 출력을 검토하세요. 다음 그림과 유사하게 표시되어야 합니다.
...
1. 렌더링된 테이블 상단에는 **테이블**과 **+ 새 차트**의 두 개의 탭이 있습니다. **+ 새 차트**를 선택합니다.
1. 새 시각화를 만들기 위해 차트 오른쪽에 있는 **직접 만들기** 옵션을 선택합니다.
1. 다음 차트 설정을 선택합니다.
   *   **Chart Type**: `Box plot`
   *   **Y-axis**: `Y`
1. 레이블 열 `Y`의 분포를 보여주는 출력을 검토하세요.
...
1. Notebook 리본에서 **Data Wrangler**를 선택한 다음, `df` 데이터 세트를 선택합니다. Data Wrangler가 실행되면 **요약** 패널에 dataframe의 설명적인 개요를 생성합니다.
...
1. Data Wrangler에서 `Y` 열을 선택합니다. `220-240` bin에서 빈도가 감소하는 것에 주목하세요. 75번째 백분위수 `211.5`는 히스토그램에서 두 영역의 전환과 대략적으로 일치합니다. 이 값을 저위험 및 고위험의 임계값으로 사용해 봅시다.
1. **작업** 패널로 이동하여 **수식**을 확장한 다음, **수식에서 열 만들기**를 선택합니다.
1. 다음 설정으로 새 열을 만듭니다.
   *   **Column name**: `Risk`
   *   **Column formula**: `(df['Y'] > 211.5).astype(int)`
1. **적용**을 선택합니다.
1. 미리 보기에 추가된 새 열 `Risk`를 검토하세요. 값 `1`을 가진 행의 수가 전체 행의 약 25%여야 함을 확인합니다 (`Y`의 75번째 백분위수이므로).
1. **notebook에 코드 추가**를 선택합니다.
1. Data Wrangler에 의해 생성된 코드를 포함하는 cell을 실행하세요.
1. `Risk` 열이 예상대로 구성되었는지 확인하기 위해 새 cell에서 다음 코드를 실행하세요.
...
1. Notebook에 새 코드 cell을 추가하고, 다음 코드를 입력한 다음 실행합니다.
...
1. `diabetes-regression` experiment를 선택하여 엽니다.

   > **팁:**
   > 기록된 experiment 실행이 보이지 않으면 페이지를 새로 고치세요.

1. **실행 메트릭**을 검토하여 회귀 모델의 정확도를 탐색하세요.
1. 홈 페이지로 돌아가서 `diabetes-classification` experiment를 선택하여 엽니다.
1. **실행 메트릭**을 검토하여 분류 모델의 정확도를 탐색하세요. 다른 유형의 모델을 학습했으므로 메트릭 유형이 다름에 유의하세요.
...
1. 모델이 생성될 때 화면 오른쪽 상단에 표시되는 알림에서 **ML 모델 보기**를 선택합니다. 창을 새로 고칠 수도 있습니다. 저장된 모델은 **ML 모델 버전** 아래에 연결됩니다.
...
1. Notebook의 **이름**을 **모델 학습 및 비교**로 설정한 다음, 설정 창을 닫습니다.
...
1. 도구 모음의 **...** 메뉴에서 **워크스페이스 설정**을 선택합니다.
1. **일반** 섹션에서 **이 워크스페이스 제거**를 선택합니다.