...
2.  왼쪽 메뉴 모음에서 **Workspaces** (아이콘은 &#128455;와 비슷하게 생겼습니다.)를 선택합니다.
...
2.  첫 번째 셀(현재는 *코드* 셀입니다)을 선택하고, 셀의 오른쪽 상단에 있는 동적 도구 모음에서 **M&#8595;** 버튼을 사용하여 셀을 *Markdown* 셀로 변환합니다.

    셀이 Markdown 셀로 변경되면 셀에 포함된 텍스트가 렌더링됩니다.

3.  필요한 경우, **&#128393;** (편집) 버튼을 사용하여 셀을 편집 모드로 전환한 다음, 내용을 삭제하고 다음 텍스트를 입력합니다.
...
2.  다음 코드를 입력하여 데이터 세트를 dataframe에 로드합니다.
...
3.  셀 왼쪽의 **&#9655; 셀 실행** 버튼을 사용하여 셀을 실행합니다. 또는 키보드에서 `SHIFT` + `ENTER`를 눌러 셀을 실행할 수 있습니다.

    > **참고**: 이번 세션에서 Spark 코드를 처음 실행하므로 Spark Pool이 시작되어야 합니다. 이는 세션에서 첫 번째 실행이 완료되는 데 1분 정도 걸릴 수 있음을 의미합니다. 후속 실행은 더 빠를 것입니다.

4.  셀 출력 아래에 있는 **+ 코드** 아이콘을 사용하여 Notebook에 새 코드 셀을 추가하고, 다음 코드를 입력합니다.
...
5.  셀 명령이 완료되면 셀 아래의 출력을 검토합니다. 다음과 유사하게 보일 것입니다.
...
2.  `df` 데이터 세트를 선택합니다. Data Wrangler가 시작되면 **요약** 패널에 dataframe에 대한 설명적 개요를 생성합니다.
3.  **Revenue** Feature를 선택하고 이 Feature의 데이터 분포를 관찰합니다.
4.  **요약** 사이드 패널의 세부 정보를 검토하고 통계 값을 관찰합니다.
...
2.  **Operations** 패널로 이동하여 **찾기 및 바꾸기**를 확장한 다음, **찾기 및 바꾸기**를 선택합니다.
...
4.  **적용**을 선택합니다.
5.  **Operations** 패널로 돌아가서 **형식**을 확장합니다.
6.  **첫 글자 대문자로 변환**을 선택합니다. **모든 단어 대문자로 변환** 토글을 켜고, **적용**을 선택합니다.
7.  **Notebook에 코드 추가**를 선택합니다. 또한, 코드를 복사하여 변환된 데이터 세트를 CSV 파일로 저장할 수도 있습니다.
...
3.  **Operations** 패널에서 **수식**을 확장한 다음, **원-핫 인코딩**을 선택합니다.
4.  **원-핫 인코딩** 패널에서 **적용**을 선택합니다.
...
2.  **Operations** 패널로 돌아가서 **정렬 및 필터링**을 확장합니다.
3.  **필터**를 선택합니다.
...
5.  **적용**을 선택하고 Data Wrangler 디스플레이 그리드의 변경 사항을 확인합니다.
...
7.  **Operations** 패널로 돌아가서 **정렬 및 필터링**을 확장합니다.
8.  **값 정렬**을 선택합니다.
...
1.  `df` dataframe에 대해 Data Wrangler를 시작합니다.
2.  **Operations** 패널로 돌아가서 **그룹화 및 집계**를 선택합니다.
3.  **그룹화할 열** 패널에서 `Brand` Feature를 선택합니다.
4.  **집계 추가**를 선택합니다.
5.  **집계할 열** 속성에서 `Revenue` Feature를 선택합니다.
6.  **집계 유형** 속성으로 `평균`을 선택합니다.
7.  **적용**을 선택합니다.
8.  **코드를 클립보드에 복사**를 선택합니다.
9.  코드를 생성하지 않고 Data Wrangler를 종료합니다.
10. `clean_data(df)` 함수 내의 `Brand` 변수 변환 코드와 집계 단계에서 생성된 코드를 결합합니다. 최종 코드 블록은 다음과 같아야 합니다.
...
2.  Notebook의 **이름**을 **Data Wrangler를 사용하여 데이터 전처리**로 설정한 다음, 설정 창을 닫습니다.
3.  Notebook 메뉴에서 **세션 중지**를 선택하여 Spark Session을 종료합니다.
...
2.  도구 모음의 **...** 메뉴에서 **Workspace 설정**을 선택합니다.
